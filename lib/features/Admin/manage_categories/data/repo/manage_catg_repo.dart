...
class ManageCatgRepo {
  ...
  Future<ApiResult< SubCategoryResponse>> createSubCategory(
    ...
    try {
      final response = await _manageCatgSource.createSubCategory(name,categry ,token);
      final  SubCategoryResponse newUser =  SubCategoryResponse.fromJson(response["data"]);
      print(newUser.name);
      return ApiResult.success(newUser);
    } catch (e) {
      return ApiResult.failure(ErrorHandler.handle(e));
